...
OSStatus CCDAudioUnitInputCallback(void *                       inRefCon,
...
- (BOOL)prepareToRecord
{
    @try {
        ...
        audioComponent = AudioComponentFindNext(NULL, &inputCompDesc);
        ...
        UInt32 flags = 1;
        ret = AudioUnitSetProperty(_audioUnit, kAudioOutputUnitProperty_EnableIO, kAudioUnitScope_Input, 1, &flags, sizeof(flags));
        if (ret != noErr) {
            return NO;
        }
        
        ret = AudioUnitSetProperty(_audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, 1, &outStreamDes, sizeof(outStreamDes));
        if (ret != noErr) {
            return NO;
        }
        
        ret = AudioUnitSetProperty(_audioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 1, &outStreamDes, sizeof(outStreamDes));
        if (ret != noErr) {
            return NO;
        }
        
        AURenderCallbackStruct callback;
        callback.inputProc = CCDAudioUnitInputCallback;
