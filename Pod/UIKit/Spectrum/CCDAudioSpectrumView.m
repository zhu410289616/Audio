...
#import "CCDAudioSpectrumView.h"

CGFloat CCDAudioTranslateAmplitudeToYPosition(CGFloat amplitude,
                                              CGRect contarinerRect,
                                              CGFloat topSpace,
                                              CGFloat bottomSpace) {
    CGFloat height = CGRectGetHeight(contarinerRect);
    CGFloat barHeight = amplitude * (height - bottomSpace - topSpace);
    return height - bottomSpace - barHeight;
}

@interface CCDAudioSpectrumView ()
...
@implementation CCDAudioSpectrumView
...
- (void)updateSpectra:(NSArray *)spectra withStype:(CCDAudioSpectraStyle)style {
    ...
    for (int i = 0; i < count; i++) {
        ...
        CGFloat y = CCDAudioTranslateAmplitudeToYPosition((CGFloat)[spectra[0][i] floatValue], self.bounds, self.topSpace, self.bottomSpace);
        CGRect rect = CGRectMake(x, y, self.barWidth, CGRectGetHeight(self.bounds) - self.bottomSpace -y);
        UIBezierPath *bar;
        switch (style) {
            case CCDAudioSpectraStyleRect:
                bar = [UIBezierPath bezierPathWithRect:rect];
                break;
            case CCDAudioSpectraStyleRound:
    ...
    if (spectra.count >= 2) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            CGFloat y = CCDAudioTranslateAmplitudeToYPosition((CGFloat)[spectra[1][i] floatValue], self.bounds, self.topSpace, self.bottomSpace);
            CGRect rect = CGRectMake(x, y, self.barWidth, CGRectGetHeight(self.bounds) - self.bottomSpace -y);
            UIBezierPath *bar;
            switch (style) {
                case CCDAudioSpectraStyleRect:
